...
/*
  Função changeStatus: Troca primeiro byte do arquivo para o status desejado
  para então manejar eventuais erros de processamento do arquivo 
  Parâmetros:
  FILE* entrada -> arquivo a ser alterado
  char status -> valor a ser escrito '0' para status de falha e '1' para status
  de sucesso
*/
...
/*
   ...
   cabeçalho FILE* entrada -> Arquivo a ser lido 
   int arqEhBinario -> operando que decide qual método de leitura será utilizado, sendo 1 para arquivo binário e 0 para arquivo de texto
*/
...
/*
   ...
   cabeçalho e depois a completa com '@' 
   Parâmetros: Pagina *p-> estrutura de dados que guarda vetor de 32000 bytes, que representa conteúdo da página de
   disco, e o número de páginas 
...
Cabecalho escreveCabecalho(Pagina* p, FILE* entrada, FILE* saida,
  ...
  fseek(entrada, 0, SEEK_SET);
  Cabecalho cab;
  criaCabecalho(&cab, entrada, arqEhBinario);
...
/*
   ...
   Parâmetro: 
   FILE* entrada: arquivo a ser checado
*/
...
/*
   ...
   um arquivo binário 
   Parâmetros: FILE *fEntrada -> arquivo de entrada a ser lido 
...
FILE* escreveBin(FILE* fEntrada, Pagina* p) {
  ...
  Cabecalho c = escreveCabecalho(p, fEntrada, saida, 0);
...
/*
   Função getReg: Lê registros de arquivo binário e os retorna com seus
    respectivos tratamento de valores nulos Parâmetros: 
    FILE *entrada -> arquivo lido 
...
Dados* getReg(FILE* entrada, Dados* d, int* nPags, int* nRegs, long* nBytes,
  ...
  int endOfPage = 0;
  char* salario = calloc(50, sizeof(char));
  strcpy(salario,"");
...
/*
  Função searchRegs: Procura registro em um arquivo binário e o imprime de
   acordo com campo passado e valor do campo 
   Parâmetros: FILE *entrada -> arquivo lido 
   char *campo -> qual campo será usado para a busca 
   char *val -> valor a ser buscado
   int printNow -> define se o páginas de disco devem ser impressas no mometo
   Retorno:
   int nPags -> número de páginas de disco acessadas
*/
...
/*
  ...
  de removido (*) 
  Parâmetros: FILE* entrada -> arquivo a ser alterado 
  long offset -> posição inicial do registro no arquivo
*/
...
/*
  ...
  encadeamento dado pela Lista 
  Parâmetros: 
  FILE* entrada -> arquivo a ser alterado 
  Lista* l -> lista de encadeamento dos registros
*/
...
/*
  ...
  para valores nulos Parâmetros: 
  FILE* entrada -> arquivo a ser alterado Dados
  dadoLido -> Dados do registro a serem escritos no arquivo 
  int isAtEnd -> Indica se registro será escrito no registro no final(1) ou não(0)
  int atNewFile -> Indica se registro será escrito em um novo arquivo
*/
void writeReg(FILE* entrada, Dados dadoLido, int isAtEnd, int atNewFile) {
  char removido = '-', remRead, tag1 = 'n', tag2 = 'c';
  long noOff = -1;
  fseek(entrada, -1, SEEK_CUR);
  fwrite(&removido, sizeof(char), 1, entrada);
  if (isAtEnd || atNewFile)
...
/*
  ...
  Parâmetros:
  FILE* entrada -> arquivo a ser alterado 
  Lista* l -> lista dos registros removidos
*/
...
/*
  ...
  Parâmetros:
  FILE* entrada -> arquivo a ser alterado Dados*
  dadosLidos -> vetor com todos os registros lidos 
  int n -> número de registros no vetor
*/
void insertOnRemoved(FILE* entrada, Dados* dadosLidos, int n) {
  ...
  if (l->tam) {
    ...
    while (aux && i < n) {
      ...
      if (dadosLidos[i].tamanhoRegistro > biggestSize) {
        ...
        writeReg(entrada, dadosLidos[i], 1, 0);
        dadosLidos[i].encadeamentoLista = ftell(entrada) -(dadosLidos[i].tamanhoRegistro +5);
      } else {
        fseek(entrada, aux->offset, SEEK_SET);
        fread(&remRead, sizeof(char), 1, entrada);
        ...
        if (remRead == '*' &&
          ...
          writeReg(entrada, dadosLidos[i], 0, 0);
          //Guarda offset em encadeamneto lista para uso na funcionalidade 13
          dadosLidos[i].encadeamentoLista = aux->offset;
          removeNode(l, aux);
          aux = l->ini;
          hasFound = 1;
          i++;
  ...
  } else {  // Caso não haja lista de encadeamento, todos os registros serão
            // inseridos no final do arquivo
    for (i = 0; i < n; i++) {
      fseek(entrada, 1, SEEK_END);
      writeReg(entrada, dadosLidos[i], 1, 0);
      //Offset guardado para uso na funcionalidade 13
      dadosLidos[i].encadeamentoLista = ftell(entrada) -(dadosLidos[i].tamanhoRegistro +5);
    }
  }
  fseek(entrada, 1, SEEK_SET);
  ...
  switch (campoAtualiza) {
    ...
    case 'c':  // Atualização do cargo
      ...
      if (strlen(valAtualiza) + 2 <= d->tamCargoServidor) {
        ...
        if (newTam >
            biggestSize) {  // Caso registro seja inserido no final do arquivo
          d->tamanhoRegistro = newTam;
          fseek(entrada, 1, SEEK_END);
          writeReg(entrada, *d, 1, 0);
          d->tamanhoRegistro = oldTam;
...
/*
  Função recalculaTamanho: recalcula o tamanho de registro e de campos de tamanho de nome e de cargo do servidor
  Parâmetros:
  Dados* d -> Registro a ser alterado
*/
void recalculaTamanho(Dados *d){
  int cargoOK = 1, nomeOk = 1;

  if (d->cargoServidor == NULL) {
    // Flag para cálculo do tamanho do cargo
    cargoOK = 0;
  ...
  // Caso nome seja nome
  if (d->nomeServidor == NULL) {
    // Flag para cálculo do tamanho do nome
    nomeOk = 0;
    d->tamNomeServidor = 0;
  } else {
    d->tamNomeServidor = strlen(d->nomeServidor) + 2;
  ...
  // Recálculo do tamanho do registro para facilitar tratamento futuro de
  // final de página
  d->tamanhoRegistro = 34 + nomeOk * (d->tamNomeServidor + 4) +
                           cargoOK * (d->tamCargoServidor + 4);
  // Reinicia flags
  cargoOK = 1;
  nomeOk = 1;
}
/*
  Função getAllReg: Lê todos os registros não removidos de um arquivo e os retorna em uma array de registros
  Parâmetros: 
  FILE* entrada -> arquivo a ser lido
...
Dados* getAllReg(FILE* entrada, int* totalREgs) {
  int i = 0;
  int nPags = 1, nRegs = 0, cargoOK = 1, nomeOk = 1, nRems=0;
  long nBytes = 0, offset=32000;
  char tag1 = 'n', tag2 = 'c';
  fseek(entrada, 32000, SEEK_SET);
  ...
  do {
    if (d->removido == '-') {
      // Caso cargo seja nulo
      res[i] = *d;
      recalculaTamanho(&res[i]);
      //Campo de encadeamento lista usado para guardar offset para uso na funcionalidade 10
      res[i].encadeamentoLista = offset;
      // Armazena registro lido e realoca vetor
      i++;
      res = realloc(res, (i + 1) * sizeof(Dados));
    }else{
        nRems++;
    }
    offset+=d->tamanhoRegistro + 5;
  ...
  *totalREgs = nRegs - nRems;
  return res;
}


/*
  Função compareIds: Função de comparação de 2 ids para ser utilizada na função de merge sort
  Parâmetros:
  const void* A -> endereço do primeiro registro
  const void* B -> endereço do segundo registro
...
/*
  Função writeNewDiskPage: preenche o final de uma página de disco, completa-a com @ e altera o 
  tamanho do último registro da página
  Parâmetros:
  FILE* file -> arquivo a ser alterado
  Dados* d-> registro a ser alterado
  long offset -> byte offset atual do arquivo
  Retorno:
  int nAts -> o número de arrobas preenchidos
*/
int writeNewDiskPage(FILE* file, Dados* d, long offset){
    int nAts=0;
    char at='@';
    for (int j = offset % 32000; j < 32000; j++) {
      fwrite(&at, sizeof(char), 1, file);
      nAts++;
    }
    d->tamanhoRegistro += nAts;
    fseek(file, -d->tamanhoRegistro - 4, SEEK_CUR);
    fwrite(&d->tamanhoRegistro, sizeof(int), 1, file);
    return nAts;
}

/*
  Função writeAllRegs: Escreve todos os registros de um array em um novo arquivo
  Parâmetros:
  FILE* newFile -> novo arquivo criado
  Dados* allRegs -> array contendo todos os registros
  int total -> total de registros contidos no array
*/
void writeAllRegs(FILE* newFile, Dados* allRegs, int total) {
  long nWrittenBytes = 0, noOff=-1;
  fseek(newFile, 1, SEEK_SET);
  fwrite(&noOff, sizeof(long), 1, newFile);
  char at = '@';
  fseek(newFile, 32000, SEEK_SET);
  for (int i = 0; i < total; i++) {
    //Caso o registro escrito ainda caiba dentro da página de disco
    if ((nWrittenBytes % 32000) + allRegs[i].tamanhoRegistro + 5 <= 32000) {
      fseek(newFile, 1, SEEK_CUR);
      writeReg(newFile, allRegs[i], 0, 1);
      nWrittenBytes += allRegs[i].tamanhoRegistro + 5;
    } else {
      //Registro anterior é modificado
      int nAts = writeNewDiskPage(newFile, &allRegs[i-1], nWrittenBytes);
      fseek(newFile, 1, SEEK_END);
      //Novo registro escrito
      writeReg(newFile, allRegs[i], 0, 1);
      nWrittenBytes += allRegs[i].tamanhoRegistro + 5 + nAts; //Adiciona-se o número de arrobas ao offset
    }
  }
}

/*
  Função copyDados: Copia todos os dados de um registro para um outro
  Parâmetros:
  Dados* dest -> registro destino
  Dados* src -> regsitro fonte
*/
void copyDados(Dados* dest, Dados* src){
    if(dest->nomeServidor) free(dest->nomeServidor);
    if(dest->cargoServidor) free(dest->cargoServidor);
    if(src->tamNomeServidor){
        dest->nomeServidor = calloc(src->tamNomeServidor, sizeof(char));
        memcpy(dest->nomeServidor, src->nomeServidor, src->tamNomeServidor-1);
    }
    if(src->tamCargoServidor){
        dest->cargoServidor = calloc(src->tamCargoServidor, sizeof(char));
        memcpy(dest->cargoServidor, src->cargoServidor, src->tamCargoServidor-1);
    }
    dest->idServidor = src->idServidor;
    dest->encadeamentoLista = src->encadeamentoLista;
    dest->removido = src->removido;
    dest->salarioServidor = src->salarioServidor;
    dest->tamanhoRegistro = src->tamanhoRegistro;
    dest->tamCargoServidor = src->tamCargoServidor;
    dest->tamNomeServidor = src->tamNomeServidor;
    strcpy(dest->telefoneServidor, src->telefoneServidor);
}

/*
  Função merging: Realiza operação registro a registro de merging entre 2 arquivos e escreve os registros resultantes em
  um arquivo de saída
  Parâmetros:
  FILE* entrada -> primeiro arquivo a ser lido
  FILE* entrada2 -> segundo arquivo a ser lido
  FILE* saida -> arquivo a ser escrito
*/
void merging(FILE* entrada, FILE* entrada2, FILE* saida){
    int nPags1=1, nPags2=1, nRegs1=0, nRegs2=0;
    long nBytes1=0, nBytes2=0, offset=32000;
    char tagN1='n', tagN2='n', tagC1='c', tagC2='c';
    //pula a página de cabeçalho de ambos arquivos
    fseek(entrada, 32000, SEEK_SET);
    fseek(entrada2, 32000, SEEK_SET);
    Dados *reg1 = getReg(entrada, reg1, &nPags1, &nRegs1, &nBytes1, &tagN1, &tagC1, 0);
    Dados *reg2 = getReg(entrada2, reg2, &nPags2, &nRegs2, &nBytes2, &tagN2, &tagC2, 0);
    Dados *ant;
    ant = calloc(1, sizeof(Dados));
    
    while(reg1 && reg2){
      //Corrige tamanhos de ambos registros
      recalculaTamanho(reg1);
      recalculaTamanho(reg2);
      fseek(saida, 1, SEEK_CUR);

      if(reg1->idServidor < reg2->idServidor){
        //Caso registro não caiba na página de disco atual
        if(offset%32000 + reg1->tamanhoRegistro + 5 > 32000){
          fseek(saida, 0, SEEK_END);
          offset += writeNewDiskPage(saida, ant, offset);
          fseek(saida, 1, SEEK_END);
        }
        writeReg(saida, *reg1, 0, 1);
        offset+=reg1->tamanhoRegistro +5;
        if(ant){
            destroiDados(ant);
            ant = calloc(1, sizeof(Dados));
        }
        //Registro que guarda o registro anterior é atualizado
        copyDados(ant, reg1);
        destroiDados(reg1);
        reg1 = getReg(entrada, reg1, &nPags1, &nRegs1, &nBytes1, &tagN1, &tagC1, 0);

      }else if(reg1->idServidor > reg2->idServidor){
        if(offset%32000 + reg2->tamanhoRegistro + 5 > 32000){
          fseek(saida, 0, SEEK_END);
          offset += writeNewDiskPage(saida, ant, offset);
          fseek(saida, 1, SEEK_END);
        }
        writeReg(saida, *reg2, 0, 1);
        offset += reg2->tamanhoRegistro +5;
        if(ant){
            destroiDados(ant);
            ant = calloc(1, sizeof(Dados));
        }
        copyDados(ant, reg2);
        destroiDados(reg2);
        reg2 = getReg(entrada2, reg2, &nPags2, &nRegs2, &nBytes2, &tagN2, &tagC2, 0);
      }else{
        if(offset%32000 + reg1->tamanhoRegistro + 5 > 32000){
          fseek(saida, 0,SEEK_END);
          offset += writeNewDiskPage(saida, ant, offset);
    ...
    //Escrita de registros que ainda sobraram no arquivo
    if(reg1){
      while(reg1){
        fseek(saida, 1, SEEK_CUR);
        if (offset % 32000 + reg1->tamanhoRegistro + 5 > 32000) {
      ...
      while(reg2){
        ...
        copyDados(ant, reg2);
        destroiDados(reg2);
        reg2 = getReg(entrada2, reg2, &nPags2, &nRegs2, &nBytes2, &tagN2,
                      &tagC2, 0);
      }
    }
    //Libera registro anterior
    destroiDados(ant);
}

/*
  Função matching: Realiza a operação de matching entre 2 arquivos de entrada e
  escreve os registros resultantes no arquivo de saída Parâmetros: FILE* entrada
  -> primeiro arquivo a ser lido FILE* entrada2 -> segundo arquivo a ser lido
  FILE* saida -> arquivo a ser escrito
*/
void matching (FILE* entrada, FILE* entrada2, FILE* saida) {
  ...
  fseek(entrada2, 32000, SEEK_SET);
  Dados* reg1 =
      getReg(entrada, reg1, &nPags1, &nRegs1, &nBytes1, &tagN1, &tagC1, 0);
  Dados* reg2 =
      getReg(entrada2, reg2, &nPags2, &nRegs2, &nBytes2, &tagN2, &tagC2, 0);
  Dados* ant;
  ant = calloc(1, sizeof(Dados));

  while (reg1 && reg2) {

    //Correção de tamanho de ambos registro
    recalculaTamanho(reg1);
    recalculaTamanho(reg2);
    ...
    if (reg1->idServidor < reg2->idServidor) {
      ...
      fseek(saida, 1, SEEK_CUR);
      //Caso registro escrito não caiba na página de disco atual
      if (offset % 32000 + reg1->tamanhoRegistro + 5 > 32000) {
        fseek(saida, 0, SEEK_END);
        offset += writeNewDiskPage(saida, ant, offset);
        fseek(saida, 1, SEEK_END);
